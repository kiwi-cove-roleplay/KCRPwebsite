...
import { listAccountPermissions, listPermissionCatalog, listPlayerNotes, searchAccounts } from "@/lib/adminApi";
...
export default async function AdminAccountDetailPage({ params }: { params: { accountId: string } }) {
  ...
  const account = matches.find((row) => row.account_id === accountId);
  if (!account) {
    notFound();
  }
